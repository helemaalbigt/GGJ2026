...
public class BuildManager : MonoBehaviour
{
	...
	private void DoHoverCheck()
	{
		...
		if (Physics.Raycast(ray, out _interactionHit, raycastDistance, grabbableMask))
		{
	...
	{
		Ray ray = Camera.main.ScreenPointToRay(_mousePos);
		if (Physics.Raycast(ray, out _movementHit, raycastDistance, mouseMoveMask))
		{
	...
	private void UpdateGroundCursor()
	{
		if (_grabbedBlock != null)
		{
			if (Physics.Raycast(_grabbedBlock.GetBottomPos(0.015f), Vector3.down, out RaycastHit hit, 10f, groundRefMask))
			{
